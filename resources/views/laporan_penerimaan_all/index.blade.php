...
<ul class="nav nav-tabs">
    <li role="presentation"> <a href="{{ route('penerimaan.laporan') }}">
        <span>Penerimaan per Aset</span>
    </a></li>
    <li role="presentation" class="active"><a href="{{ route('penerimaan.allRencana') }}">
...
<script>
    ...
    $("#cari").on("click", function (event) {
        ...
        table.ajax.url("/penerimaan/"+ tanggal_awal + '/' + tanggal_akhir);
        table.ajax.reload();
    });

    function getTotal(tanggal_awal, tanggal_akhir){
        $.ajax({
            url: '/penerimaan/total/'+tanggal_awal+'/'+tanggal_akhir,
            type: 'GET',
            dataType: 'json',
