...
<body>
    <table width="100%">
        ...
        <h1 class="text-center">Laporan Penerimaan Sparepart Kendaraan {{ $member->kode_kabin }}</h1>
    ...
    <table width="100%" style="margin-top: 15px">
        <tr>
            <td width="10%">Kode Penerimaan : {{ $item->nomor_terima ?? '' }}</td>
        </tr>  
        <tr>
        ...
        <tbody>
            @foreach ($detail as $item)
                <tr>
                    ...
                    <td class="text-right">{{$item->jumlah_terima}}</td>
                    <td class="text-right">{{"Rp. " .  format_uang($item->subtotal_terima)}}</td>
                </tr>
            @endforeach
        </tbody>
        <tfoot>
